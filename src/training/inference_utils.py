...
def generate_videos(G: torch.nn.Module, z: torch.Tensor, c: torch.Tensor) -> torch.Tensor:
    ...
    camera_cfg = dnnlib.EasyDict({'name': 'front_circle', 'num_frames': 64, 'yaw_diff': 0.5, 'pitch_diff': 0.3, 'use_zoom': True})
...
def save_videos(videos: torch.Tensor, save_path: os.PathLike):
    ...
    tv.io.write_video(save_path, video, fps=25, video_codec='h264', options={'crf': '10'})
...
def generate_camera_angles(camera, default_fov: Optional[float]=None):
    if camera.name == 'front_circle':
        ...
        angles = np.stack([pitch, yaw, np.zeros(camera.num_frames)], axis=1) # [num_frames, 3]
