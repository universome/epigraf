...
def resize_dataset(source_dir, target_dir: str=None, size: int=None, format: str='.jpg', num_jobs: int=8, ignore_regex: str=None, ignore_ext: str=None, images_only: bool=False, fname_prefix: str=''):
    ...
    for file_name in tqdm(file_names, desc=f'Collecting jobs'):
        src_path = os.path.join(source_dir, file_name)

        if file_ext(src_path) in Image.EXTENSION:
            trg_path = os.path.join(target_dir, fname_prefix + file_name[:file_name.rfind('.')] + format)
